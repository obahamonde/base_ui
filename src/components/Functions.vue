...
const useFunction = async () => {
  ...
  try {
    const namespace = props.user.ref;
...
<template>
  <div
    class="col center py-24 overflow-auto center w-full max-w-168 min-w-128"
  >
    <FunctionQueue :user="props.user">
      <template #default="{ state }">
               <div class="col center w-full mt-8">
          <component
            :is="TextBlock"
            :content="state.data"
            :mounted="false"
  ...
  <Input type="text" v-model:modelValue="message" class="input "
